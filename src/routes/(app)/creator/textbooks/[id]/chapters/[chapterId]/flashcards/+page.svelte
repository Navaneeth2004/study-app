<script lang="ts">
	...
	onMount(async () => {
		loading = true;
		try {
			const [textbook, chapter, cards] = await Promise.all([getTextbook(textbookId), getChapter(chapterId), listByChapter(chapterId)]);
			textbookTitle = textbook.title; chapterTitle = chapter.title; flashcards = cards;
	...
	async function handleBulkDelete() {
		...
		try {
			...
			selectedIds = new Set(); bulkMode = false; confirmBulkDelete = false;
	...
	function handleDragStart(e: DragEvent, id: string) { draggingId = id; e.dataTransfer?.setData('text/plain', id); }
	...
	async function handleAIInsert(result: AIGenerationResult) {
		...
		try {
			const created = await Promise.all(
				cards.map((card, i) => createFlashcard({ frontText: card.front_text, backText: card.back_text, chapter: chapterId, order: flashcards.length + i + 1 }))
			);
...
<svelte:head><title>Flashcards — {chapterTitle || 'Chapter'}</title></svelte:head>
...
<div class="flex flex-col gap-6">
	...
	<div class="flex gap-1 border-b border-[var(--color-surface-700)]">
		<a href="/creator/textbooks/{textbookId}/chapters/{chapterId}" class="px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors border-transparent text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)]">Content</a>
		<a href="/creator/textbooks/{textbookId}/chapters/{chapterId}/flashcards" class="px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors border-[var(--color-accent-500)] text-[var(--color-accent-400)]">Flashcards</a>
	</div>
	...
	<div class="flex items-center justify-between gap-4">
		...
		<div class="flex shrink-0 items-center gap-2">
			{#if bulkMode}
				...
				<button onclick={() => { selectedIds = new Set(flashcards.map((c) => c.id)); }} class="text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] transition-colors">All</button>
				<button onclick={() => { selectedIds = new Set(); }} class="text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] transition-colors">None</button>
			...
			{:else}
				...
				<button onclick={() => (showAIModal = true)}
					class="flex items-center gap-2 rounded-xl border border-[var(--color-surface-600)] px-4 py-2.5 text-sm font-medium text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors">
	...
	{#if loading}
		<div class="flex flex-col gap-2">
			{#each Array(3) as _}<div class="h-12 rounded-xl border border-[var(--color-surface-700)] bg-[var(--color-surface-900)]"></div>{/each}
		</div>
	{:else if flashcards.length === 0}
		<EmptyState heading="No flashcards yet" description="Add flashcards to this chapter." />
	{:else if bulkMode}
		<div class="flex flex-col gap-2">
