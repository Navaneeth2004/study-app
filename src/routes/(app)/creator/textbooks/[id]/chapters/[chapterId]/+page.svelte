...
{:else}
	<div class="flex flex-col gap-6 max-w-2xl">
		...
		<nav class="flex items-center gap-2 text-sm">
			...
			<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" class="text-[var(--color-text-muted)]">
		...
		<div class="flex gap-1 border-b border-[var(--color-surface-700)]">
			<a
				href="/creator/textbooks/{textbookId}/chapters/{chapterId}"
			...
			<a
				href="/creator/textbooks/{textbookId}/chapters/{chapterId}/flashcards"
				class="px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors
		...
		<div role="list" class="flex flex-col gap-3">
			{#each blocks as block (block.id)}
				{#if previewMode}
					<div class="py-1">
						{#if block.type === 'title'}
							<TitleBlockRenderer data={block.data} />
						{:else if block.type === 'subtitle'}
							<SubtitleBlockRenderer data={block.data} />
						{:else if block.type === 'paragraph'}
							<ParagraphBlockRenderer data={block.data} />
						{:else if block.type === 'bullet_list'}
							<BulletListRenderer data={block.data} />
						{:else if block.type === 'table'}
							<TableBlockRenderer data={block.data} />
						{:else if block.type === 'image'}
							<ImageBlockRenderer data={block.data} />
						{:else if block.type === 'audio'}
							<AudioBlockRenderer data={block.data} />
						{:else if block.type === 'divider'}
							<DividerBlockRenderer />
						{:else if block.type === 'callout'}
							<CalloutBlockRenderer data={block.data} />
						{:else if block.type === 'video'}
							<VideoBlockRenderer data={block.data} />
						{/if}
					</div>
				{:else}
					<BlockWrapper
			...
			<div class="relative">
				...
				{#if showAIPicker}
					<div
						...
						onkeydown={(e) => e.key === 'Escape' && (showAIPicker = false)}
					></div>
					<div class="absolute bottom-full left-0 z-20 mb-2 w-full overflow-hidden rounded-xl
						...
						{#each AI_OUTPUT_TYPES as { type, label }}
							<button
								onclick={() => { showAIPicker = false; aiOutputType = type; showAIModal = true; }}
