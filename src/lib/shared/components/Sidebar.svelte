<script lang="ts">
	...
	function isActive(href: string): boolean {
		...
		if (href === '/creator') return path === '/creator' || path.startsWith('/creator/textbooks');
...
>
	...
	<div class="border-t border-[var(--color-surface-700)] p-4">
		<div class="flex items-center gap-2 rounded-lg bg-[var(--color-surface-800)] px-3 py-2">
			<div class="h-2 w-2 rounded-full {$roleStore === 'creator' ? 'bg-[var(--color-success-500)]' : 'bg-[var(--color-text-muted)]'}"></div>
