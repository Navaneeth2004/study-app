<script lang="ts">
	...
	onMount(async () => {
		try {
			bookmarkId = await isBookmarked(contentType, contentId);
		} finally {
			loading = false;
		}
	});
...
<div class="relative" bind:this={wrapperEl}>
	<button
		...
		class="flex h-8 w-8 items-center justify-center rounded-lg transition-colors
		       {bookmarkId
			? 'text-[var(--color-accent-400)] hover:bg-[var(--color-accent-500)]/10'
	...
	{#if showFolderPicker}
		...
		<div
			class="absolute right-0 top-9 z-30 min-w-44 overflow-hidden rounded-xl border
			       border-[var(--color-surface-700)] bg-[var(--color-surface-800)] shadow-2xl"
			onclick={(e) => e.stopPropagation()}
		>
			<p class="border-b border-[var(--color-surface-700)] px-3 py-2 text-xs font-semibold
			          uppercase tracking-widest text-[var(--color-text-muted)]">
				Save to…
			</p>
	...
	{#if showConfirm}
		...
		<!-- svelte-ignore a11y_no_static_element_interactions -->
		<div
			class="absolute right-0 top-9 z-30 flex items-center gap-2 rounded-xl border
