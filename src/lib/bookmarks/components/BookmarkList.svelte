<script lang="ts">
	...
	/** Determine navigation path from bookmark, handling missing metadata gracefully */
	function getNavPath(bm: Bookmark): string | null {
		switch (bm.contentType) {
			case 'textbook':
				return `/viewer/textbooks/${bm.contentId}`;
			case 'chapter': {
				const tbId = bm.contentMeta?.textbookId;
				if (!tbId) return null; // can't navigate without textbookId
				return `/viewer/textbooks/${tbId}/chapters/${bm.contentId}`;
			}
			case 'flashcard_category':
				return `/viewer/flashcards/category/${bm.contentId}`;
			case 'flashcard': {
				const catId = bm.contentMeta?.categoryId || bm.contentMeta?.chapterId;
				if (bm.contentMeta?.categoryId) return `/viewer/flashcards/category/${bm.contentMeta.categoryId}`;
				if (bm.contentMeta?.chapterId) return `/viewer/flashcards/chapter/${bm.contentMeta.chapterId}`;
				return null;
			}
			default:
				return null;
	...
	function canNavigate(bm: Bookmark): boolean { return getNavPath(bm) !== null; }
</script>

{#if bookmarks.length === 0}
	<div class="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-[var(--color-surface-600)] py-12 text-center">
		...
		{#each bookmarks as bm (bm.id)}
			<div role="listitem" draggable="true"
				...
				class="group flex flex-col gap-2 rounded-xl border bg-[var(--color-surface-900)] px-4 py-3 transition-colors
				       {draggingId === bm.id ? 'border-[var(--color-accent-500)] opacity-50' : 'border-[var(--color-surface-700)] hover:border-[var(--color-surface-600)]'}">
					...
					<div class="flex shrink-0 items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
						{#if canNavigate(bm)}
							<button onclick={() => onNavigate(bm)}
								class="rounded-lg border border-[var(--color-surface-600)] px-2.5 py-1 text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:border-[var(--color-surface-500)] transition-colors">
								Go to
							</button>
						{:else}
							<span class="rounded-lg border border-[var(--color-surface-700)] px-2.5 py-1 text-xs text-[var(--color-text-muted)] cursor-not-allowed" title="Navigation data missing">
								Go to
							</span>
						{/if}
						<button onclick={() => movingId = movingId === bm.id ? null : bm.id}
						...
						{#if confirmRemoveId !== bm.id}
							<button onclick={() => (confirmRemoveId = bm.id)}
				...
				{#if movingId === bm.id}
					<div class="flex flex-wrap gap-1.5 pt-1 border-t border-[var(--color-surface-700)]">
						...
						<button onclick={() => { movingId = null; onMove(bm, null); }}
							class="rounded-lg border border-[var(--color-surface-600)] px-2.5 py-1 text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors">No folder</button>
						{#each folders as folder}
							<button onclick={() => { movingId = null; onMove(bm, folder.id); }}
								class="rounded-lg border border-[var(--color-surface-600)] px-2.5 py-1 text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors">{folder.name}</button>
						{/each}
						<button onclick={() => (movingId = null)}
							class="rounded-lg border border-[var(--color-surface-600)] px-2 py-1 text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text-secondary)] transition-colors">Cancel</button>
					</div>
				{/if}
				...
				{#if confirmRemoveId === bm.id}
					<div class="flex items-center gap-2 border-t border-[var(--color-surface-700)] pt-2">
						...
						<button onclick={() => { confirmRemoveId = null; onRemove(bm.id); }}
							class="rounded-lg bg-[var(--color-error-500)]/15 px-2.5 py-1 text-xs text-[var(--color-error-400)] hover:bg-[var(--color-error-500)]/25 transition-colors">Remove</button>
						<button onclick={() => (confirmRemoveId = null)}
							class="rounded-lg border border-[var(--color-surface-600)] px-2.5 py-1 text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors">Cancel</button>
					</div>
				{/if}
			</div>
		{/each}
