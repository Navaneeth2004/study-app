<script lang="ts">
	...
	async function handleUnlock() {
		if (!password) {
			unlockState = { ...unlockState, error: 'Password is required.' };
			return;
		}
		...
		try {
			...
			unlockState = {
				loading: false,
				error: e instanceof Error ? e.message : 'Could not verify password.'
			};
		}
	}

	function handleSwitchToViewer() {
		roleStore.setViewer();
	}
</script>
		...
		<div class="rounded-xl border border-[var(--color-surface-700)] bg-[var(--color-surface-900)] p-5">
			<div class="flex items-center justify-between gap-4">
				<div class="flex flex-col gap-1">
					...
					<p class="text-sm text-[var(--color-text-secondary)]">
						You can create and manage study content.
					</p>
				</div>
				<span
					class="shrink-0 rounded-full bg-[var(--color-success-500)]/15 px-3 py-1
					       text-xs font-medium text-[var(--color-success-500)]"
				>
					Creator
				</span>
			</div>
			...
			<div class="mt-5 border-t border-[var(--color-surface-700)] pt-5">
				<button
					...
					class="rounded-lg border border-[var(--color-surface-600)] px-4 py-2
		...
		<div class="rounded-xl border border-[var(--color-surface-700)] bg-[var(--color-surface-900)] p-5">
			<div class="flex items-center justify-between gap-4">
				<div class="flex flex-col gap-1">
					...
					<p class="text-sm text-[var(--color-text-secondary)]">
						Enter your password to enable content creation.
					</p>
				</div>
				<span
					class="shrink-0 rounded-full bg-[var(--color-surface-700)] px-3 py-1
			...
			<div class="mt-5 flex flex-col gap-3 border-t border-[var(--color-surface-700)] pt-5">
				...
				<button
					...
					disabled={unlockState.loading}
					class="self-start rounded-xl bg-[var(--color-accent-500)] px-5 py-2.5
					       text-sm font-medium text-white transition-all
					       hover:bg-[var(--color-accent-400)] disabled:cursor-not-allowed
