<script lang="ts">
	...
	import type { LogData } from '$lib/calendar/calendarService';
	import UnsavedChangesModal from '$lib/shared/components/UnsavedChangesModal.svelte';

	interface Props {
		day: CalendarDay;
		saving?: boolean;
		onSave: (data: LogData & { mood: MoodRating | null }) => Promise<void>;
		...
		<div class="flex flex-col gap-4 p-5">
			...
			{:else}
				...
				<div class="flex flex-col gap-3">
					...
					<div class="flex flex-col gap-1.5">
						<label for="day-duration" class="text-xs font-medium text-[var(--color-text-secondary)]">Duration (minutes)</label>
						<input id="day-duration" type="number" bind:value={duration} min={1} max={600} placeholder="e.g. 45"
							...
							{#each MOODS as m}
								<button onclick={() => (mood = mood === m.value ? null : m.value)} title={m.label}
									class="flex h-9 w-9 items-center justify-center rounded-lg border text-base transition-colors
									       {mood === m.value ? 'border-[var(--color-accent-500)] bg-[var(--color-accent-500)]/15' : 'border-[var(--color-surface-600)] hover:border-[var(--color-surface-500)]'}">
									{m.emoji}
								</button>
							{/each}
						</div>
						...
						{#if day.log}
							...
							{:else}
								<button onclick={handleDelete} disabled={saving}
									class="ml-auto rounded-xl bg-[var(--color-error-500)]/15 px-4 py-2 text-sm
