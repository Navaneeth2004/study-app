...
export async function createChapter(
	...
	try {
		const r = await pb.collection('chapters').create({
			title: data.title,
			order,
			textbook: textbookId,
...
export async function updateChapter(id: string, data: Partial<ChapterFormData>): Promise<void> {
	try {
		await pb.collection('chapters').update(id, data);
	} catch (e) {
		if (e instanceof ClientResponseError) throw new Error(e.message);
		throw e;
	}
	...
	try {
		await Promise.all(
			chapters.map((chapter, index) =>
				pb.collection('chapters').update(chapter.id, { order: index + 1 })
			)
		);
	} catch (e) {
		if (e instanceof ClientResponseError) throw new Error(e.message);
...
export async function getChapter(id: string): Promise<import('./creatorTypes').Chapter> {
	try {
