...
<div class="flex flex-col gap-3 overflow-x-auto">
	<table class="w-full text-sm border-collapse">
		<thead>
			<tr>
					...
					<th class="p-0">
						<div class="group relative flex items-center gap-1 border-b border-[var(--color-surface-600)] pb-1">
							<input type="text" value={header}
								...
								class="flex-1 bg-transparent text-xs font-semibold uppercase tracking-wider text-[var(--color-text-secondary)] focus:outline-none min-w-16 px-1" />
							{#if data.headers.length > 1}
								<button onclick={() => removeColumn(colIndex)} aria-label="Remove column"
									class="opacity-0 group-hover:opacity-100 text-[var(--color-text-muted)] hover:text-[var(--color-error-400)] transition-all">
				...
				{/each}
				<th class="w-8 pb-1">
					<button onclick={addColumn} aria-label="Add column"
					...
					{#each row as cell, colIndex}
						<td class="p-0 py-1 pr-2">
							<input type="text" value={cell}
								...
								placeholder="—"
								class="w-full bg-transparent text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-surface-600)] focus:outline-none px-1 min-w-16" />
						</td>
					{/each}
					<td class="w-8 py-1">
						{#if data.rows.length > 1}
							<button onclick={() => removeRow(rowIndex)} aria-label="Remove row"
								class="opacity-0 group-hover:opacity-100 text-[var(--color-text-muted)] hover:text-[var(--color-error-400)] transition-all">
								<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
									<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
								</svg>
	...
	<!-- Caption field -->
	<div class="border-t border-[var(--color-surface-700)] pt-2">
		<input type="text" value={caption}
			oninput={(e) => onUpdate({ ...data, caption: (e.target as HTMLInputElement).value } as TableBlock['data'] & {caption:string})}
			placeholder="Table caption (optional)…"
			class="w-full bg-transparent text-sm text-[var(--color-text-secondary)] placeholder:text-[var(--color-text-muted)] focus:outline-none border-b border-[var(--color-surface-700)] pb-1" />
	</div>
</div>
