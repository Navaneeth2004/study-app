<script lang="ts">
	import type { SubtitleBlock } from '$lib/creator/contentTypes';

	interface Props {
		data: SubtitleBlock['data'];
		onUpdate: (data: SubtitleBlock['data']) => void;
	}
	...
	function wrapSelection(tag: string) {
		...
		setTimeout(() => { inputEl.setSelectionRange(start + tag.length + 2, start + tag.length + 2 + selected.length); inputEl.focus(); }, 0);
	}
</script>
...
<div class="flex flex-col gap-1.5">
	...
	<input
		...
		class="w-full bg-transparent text-lg font-medium text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none"
	/>
</div>
