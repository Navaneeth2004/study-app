<script lang="ts">
	...
	let { type, blockData, blockId, collapsed, onToggleCollapse, onDelete, onAddBelow, onDragStart, onDragEnd, onDrop, draggingId, children }: Props = $props();
...
<div class="group flex flex-col">
			...
			<div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
				<button onclick={onToggleCollapse}
					...
					<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
						{#if collapsed}<polyline points="6 9 12 15 18 9"/>
						{:else}<polyline points="18 15 12 9 6 15"/>{/if}
					</svg>
				</button>
	...
	<div class="relative flex items-center justify-center py-1 opacity-0 group-hover:opacity-100 transition-opacity">
		...
		<button
			bind:this={addBelowBtn}
			onclick={openAddBelow}
		...
		{#if showAddBelowPicker}
			...
			<div class="absolute left-1/2 -translate-x-1/2 z-30 w-44 overflow-hidden rounded-xl border border-[var(--color-surface-700)] bg-[var(--color-surface-800)] shadow-2xl
				...
				{#each Object.entries(BLOCK_TYPE_LABELS) as [t, label]}
					<button
						onclick={() => { showAddBelowPicker = false; onAddBelow(t as BlockType); }}
