...
export async function listTextbooks(): Promise<Textbook[]> {
	try {
		const records = await pb.collection('textbooks').getFullList({
			filter: `owner = "${pb.authStore.record?.id}"`, sort: '-created', expand: 'chapters_via_textbook', requestKey: null
		});
...
export async function getTextbook(id: string): Promise<Textbook> {
	try {
		...
		return { id: r.id, title: r.title, description: r.description ?? '', owner: r.owner, created: r.created, updated: r.updated };
	} catch (e) { if (e instanceof ClientResponseError) throw new Error(e.message); throw e; }
}

export async function createTextbook(data: TextbookFormData): Promise<Textbook> {
	try {
		const r = await pb.collection('textbooks').create({ title: data.title, description: data.description, owner: pb.authStore.record?.id });
		return { id: r.id, title: r.title, description: r.description ?? '', owner: r.owner, created: r.created, updated: r.updated };
	} catch (e) { if (e instanceof ClientResponseError) throw new Error(e.message); throw e; }
}

export async function updateTextbook(id: string, data: Partial<TextbookFormData>): Promise<void> {
	try { await pb.collection('textbooks').update(id, data); }
	catch (e) { if (e instanceof ClientResponseError) throw new Error(e.message); throw e; }
}
