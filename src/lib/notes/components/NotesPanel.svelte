<script lang="ts">
	...
	async function loadNote() {
		...
		try {
			...
			error = e instanceof Error ? e.message : 'Could not load note.';
...
>
		...
		{:else}
			{#if !note && !content}
				<p class="text-xs text-[var(--color-text-muted)]">
					No notes yet. Start typing to add your first note.
				</p>
			{/if}
			...
			<div class="flex items-center justify-between">
				...
				{#if isDirty}
					<span class="text-xs text-[var(--color-warning-400)]">Unsaved changes</span>
				{/if}
			</div>
	...
	<div class="flex shrink-0 items-center gap-2 border-t border-[var(--color-surface-700)] p-4">
		<button
			...
			class="flex-1 rounded-xl bg-[var(--color-accent-500)] py-2 text-sm font-medium
		...
		{#if note}
			...
			{:else}
				<button onclick={handleDelete} disabled={saving}
					class="rounded-xl bg-[var(--color-error-500)]/15 px-3 py-2 text-xs
					       text-[var(--color-error-400)] hover:bg-[var(--color-error-500)]/25 transition-colors">
					Delete
				</button>
				<button onclick={() => (confirmDelete = false)}
					class="rounded-xl border border-[var(--color-surface-600)] px-2 py-2 text-xs
